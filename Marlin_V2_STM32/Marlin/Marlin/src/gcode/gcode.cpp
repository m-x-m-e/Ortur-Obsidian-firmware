 ...
 */
void GcodeSuite::process_parsed_command(const bool no_ok/*=false*/ , const bool force_emergency_parse/*=false*/) {
...
//      #if DISABLED(EMERGENCY_PARSER)
        case 108:if(force_emergency_parse) M108(); break;         // M108: Cancel Waiting
        case 112:if(force_emergency_parse) M112(); break;         // M112: Full Shutdown
        case 410:if(force_emergency_parse) M410(); break;         // M410: Quickstop - Abort all the planned moves.
	  #if ENABLED(HOST_PROMPT_SUPPORT)
        case 876:if(force_emergency_parse) M876(); break;         // M876: Handle Host prompt responses
	  #endif
//      #else
