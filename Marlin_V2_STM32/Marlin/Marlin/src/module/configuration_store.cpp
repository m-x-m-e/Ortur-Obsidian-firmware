...
typedef struct SettingsDataStruct {
  ...
  //GLOBAL LEVELING
  //
  bool  planner_leveling_active;                        // M420 Sn  planner.leveling_active
...
void MarlinSettings::postprocess() {
  ...
  TERN_(HAS_LEVELING, set_bed_leveling_enabled(leveling_active_states));
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    {
      ...
      const bool leveling_active = TERN(HAS_LEVELING, planner.leveling_active, false);
      const float z_fade_height = TERN(ENABLE_LEVELING_FADE_HEIGHT, planner.z_fade_height, 10.0f);
      EEPROM_WRITE(leveling_active);
      EEPROM_WRITE(z_fade_height);
    }
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      {
        _FIELD_TEST(planner_leveling_active);
        EEPROM_READ(TERN(HAS_LEVELING, leveling_active_states, dummyf));
...
void MarlinSettings::reset() {
  ...
  // Global Leveling
  //
  TERN_(ENABLE_LEVELING_FADE_HEIGHT, new_z_fade_height = LEVELING_FADE_HEIGHT);
