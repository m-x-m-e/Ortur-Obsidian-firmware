...
void MarlinUI::update() {
...
#if ENABLED(EXTENSIBLE_UI) &&  ENABLED(ADVANCED_PAUSE_FEATURE)
  void lcd_pause_show_message(
    ...
    const uint8_t extruder/*=active_extruder*/
	  ...
	  switch (message) {
	    case PAUSE_MESSAGE_PARKING:  message_str = (GET_TEXT(MSG_PAUSE_PRINT_PARKING)); break;
	    case PAUSE_MESSAGE_CHANGING: message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_INIT)); break;
	    case PAUSE_MESSAGE_UNLOAD:   message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_UNLOAD)); break;
	    case PAUSE_MESSAGE_WAITING:  message_str = (GET_TEXT(MSG_ADVANCED_PAUSE_WAITING)); break;
	    case PAUSE_MESSAGE_INSERT:   message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_INSERT)); break;
	    case PAUSE_MESSAGE_LOAD:     message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_LOAD)); break;
	    case PAUSE_MESSAGE_PURGE:    message_str =
									 #if ENABLED(ADVANCED_PAUSE_CONTINUOUS_PURGE)
									 		(GET_TEXT(MSG_FILAMENT_CHANGE_CONT_PURGE));
									 #else
									 		(GET_TEXT(MSG_FILAMENT_CHANGE_PURGE));
									 #endif
									 break;
	    case PAUSE_MESSAGE_RESUME:   message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_RESUME)); break;
	    case PAUSE_MESSAGE_HEAT:     message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_HEAT)); break;
	    case PAUSE_MESSAGE_HEATING:  message_str = (GET_TEXT(MSG_FILAMENT_CHANGE_HEATING)); break;
	    case PAUSE_MESSAGE_OPTION:   break;
	    case PAUSE_MESSAGE_STATUS:   break;
	    default: break;
	  }

	  if(message_str)
		  MarlinUI::set_status(message_str);
  }
#endif
  ...
  void MarlinUI::abort_print() {
      ...
      card.flag.abort_sd_printing = true;
		  ...
		  //Make sure to exit the wait state
