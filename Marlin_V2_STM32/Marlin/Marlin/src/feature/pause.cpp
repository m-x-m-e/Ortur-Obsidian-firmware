...
#if HAS_HOTEND
  int16_t resume_target_temperature[HOTENDS];
#endif

#if HAS_HEATED_BED
  int16_t resume_target_temperature_bed;
#endif

#if HAS_FAN
  uint8_t resume_fan_speed[FAN_COUNT];
#endif
...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  while (ABS(thermalManager.degHotend(active_extruder) - thermalManager.degTargetHotend(active_extruder)) > TEMP_HYSTERESIS)
    idle();
...
#if EXTRUDERS
  HOTEND_LOOP() resume_target_temperature[e] = thermalManager.temp_hotend[e].target;
#endif

  TERN_(HAS_HEATED_BED, resume_target_temperature_bed = thermalManager.temp_bed.target);

#if HAS_FAN
  for (int8_t f = 0; f < FAN_COUNT; f++) resume_fan_speed[f] = thermalManager.fan_speed[f];
#endif

	//save recovery info immediately after pause
#if ENABLED(POWER_LOSS_RECOVERY)
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  while (
   ...
   #if EXTRUDERS
     HOTEND_LOOP() thermalManager.setTargetHotend(resume_target_temperature[e], e);
   #endif

      TERN_(HAS_HEATED_BED, thermalManager.setTargetBed(resume_target_temperature_bed));

      TERN_(HAS_FAN, for (uint8_t f = 0; f < FAN_COUNT; f++) thermalManager.set_fan_speed(f,resume_fan_speed[f]));
      ...
      TERN_(EXTENSIBLE_UI, if(wait_for_user){ ExtUI::onUserConfirmRequired_P(PSTR("Reheat finished."));});
