...
uint32_t PrintJobRecovery::power_loss_count = POWER_LOSS_THRESHOLD;
uint32_t PrintJobRecovery::power_loss_milli = 0;
...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
    ...
    //save paused states
    info.print_paused = IS_SD_PAUSED();
    info.resume_position = resume_position;
    ...
    #if EXTRUDERS
      HOTEND_LOOP() info.target_temperature[e] = info.print_paused ? resume_target_temperature[e] : thermalManager.temp_hotend[e].target;
    #endif

      TERN_(HAS_HEATED_BED, info.target_temperature_bed = info.print_paused ? resume_target_temperature_bed : thermalManager.temp_bed.target);

      TERN_(POWER_LOSS_RECOVERY, info.power_loss_recovery_active = enabled);

      TERN_(FILAMENT_RUNOUT_SENSOR, info.filament_runout_active = runout.enabled);

    #if HAS_FAN
      COPY(info.fan_speed, (info.print_paused ? resume_fan_speed : thermalManager.fan_speed));
    #endif
   ...
   */
  void PrintJobRecovery::_outage(bool power_loss_trigged) {
    #if ENABLED(BACKUP_POWER_SUPPLY)
    ...
    // pause status will trigger recovery info save, no need save again
    if (IS_SD_PRINTING()|| IS_SD_PAUSED())
    	save(power_loss_trigged, zraise);

    if(power_loss_trigged)
    {
...
void PrintJobRecovery::resume() {
  ...
  // Restore the bed temperature to guarante adhesion
  #if HAS_HEATED_BED
  {
  ...
  // Restore all hotend appropriate temperature to separate the nozzle and object
  #if HAS_HOTEND
    HOTEND_LOOP() {
  	  ...
  	  if (et) {
  	  	...
  	  	gcode.process_subcommands_now(cmd);
  	    #endif
  	    sprintf_P(cmd, PSTR("M109 S%i"), uint32_t(et*0.8));
  	    gcode.process_subcommands_now(cmd);
  	  }
    }
  #endif
  ...
  // Reset E, raise Z, home XY...
    ...
    sprintf_P(cmd, PSTR("G92.9 E0 "
        ...
        #else
          "Z0\nG1Z%sF30"                       // Set Z=0 and Raise Z now
        #endif
      ),
  ...
  if(info.print_paused)
  {
	// Move back to the saved print XY
	sprintf_P(cmd, PSTR("G1 X%s Y%s F3000"),
	dtostrf(info.resume_position.x, 1, 3, str_1),
	dtostrf(info.resume_position.y, 1, 3, str_2));
	gcode.process_subcommands_now(cmd);
  }
  else
  {
	// Move back to the saved last XY
	sprintf_P(cmd, PSTR("G1 X%s Y%s F3000"),
  ...
  // move backe to saved print z
  if(info.print_paused)
  {
	// Move back to the saved print Z
	dtostrf(info.resume_position.z, 1, 3, str_1);
	sprintf_P(cmd, PSTR("G1 Z%s F200"), str_1);
	gcode.process_subcommands_now(cmd);
  }
  ...
  // Restore Feedrate Percentage
  sprintf_P(cmd, PSTR("M220 S%d"), int(info.feedrate_percentage));
  gcode.process_subcommands_now(cmd);

  // Restore Flow Percentage
#if EXTRUDERS
  LOOP_L_N(e, EXTRUDERS) {
  sprintf_P(cmd, PSTR("M221 T%d S%d"),int(e),int(info.flow_percentage[e]));
  gcode.process_subcommands_now(cmd);
  }
#endif

#if ENABLED(FILAMENT_RUNOUT_SENSOR)
  sprintf_P(cmd, PSTR("M412 S%d"),info.filament_runout_active?1:0);
  gcode.process_subcommands_now(cmd);
#endif
  ...
  sprintf_P(cmd, "%s", "M21");
  gcode.process_subcommands_now(cmd); //Just in case, initialize the SD card first
...
#if ENABLED(POWER_LOSS_RECOVERY)
  sprintf_P(cmd, PSTR("M413 S%d"),info.power_loss_recovery_active?1:0);
  gcode.process_subcommands_now(cmd);
#endif
}
    ...
    if (info.valid_head) {
      if (info.valid_head == info.valid_foot) {
        ...
        DEBUG_ECHOLNPAIR("feedrate: ", info.feedrate);
          ...
          // Restore Feedrate Percentage
          DEBUG_ECHOLNPAIR("feedrate_percentage: ", info.feedrate_percentage);

          // Restore Flow Percentage
        #if EXTRUDERS
          DEBUG_ECHOPGM("flow_percentage: ");
          LOOP_L_N(e, EXTRUDERS) {
        	  DEBUG_ECHO(int(info.flow_percentage[e]));
        	  if (e < EXTRUDERS - e) DEBUG_CHAR(',');
          }
        #endif
