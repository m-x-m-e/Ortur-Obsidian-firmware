...
//#define FILAMENT_RUNOUT_SENSOR_DEBUG
  ...
  #define FILAMENT_RUNOUT_THRESHOLD 5000
#endif

#ifndef FILAMENT_RUNOUT_INTERVAL
  #define FILAMENT_RUNOUT_INTERVAL 1  //1 millisecond
#endif
...
class FilamentSensorBase {
  ...
  public:
    ...
    static inline uint8_t poll_runout_pins() {
    ...
    #elif ENABLED(FIL_RUNOUT_PULLDOWN)
	  #define _INIT_RUNOUT(N) SET_INPUT_PULLDOWN(FIL_RUNOUT##N##_PIN);
    #endif
      REPEAT_S(1, INCREMENT(NUM_RUNOUT_SENSORS), _INIT_RUNOUT);
      #undef  _INIT_RUNOUT
      ...
      uint8_t states =  (0 REPEAT_S(1, INCREMENT(NUM_RUNOUT_SENSORS), _OR_RUNOUT));
      #undef _OR_RUNOUT
...
/********************************* RESPONSE TYPE *********************************/
  ...
  class RunoutResponseDebounced {
    private:
      static constexpr uint32_t runout_threshold = FILAMENT_RUNOUT_THRESHOLD;
      static uint32_t runout_count;
      static constexpr uint32_t runout_count_interval = FILAMENT_RUNOUT_INTERVAL;
      static uint32_t runout_count_milli;
    public:
      static inline void reset()                                  { runout_count = runout_threshold; runout_count_milli = millis(); }
      static inline void run()                                    { if (runout_count > 0 && millis() >= runout_count_milli) { runout_count--; runout_count_milli = millis() + runout_count_interval; }}
      static inline bool has_run_out()                            { return runout_count <= 0; }
      static inline void block_completed(const block_t* const)    { }
      static inline void filament_present(const uint8_t)          { runout_count = runout_threshold; runout_count_milli = millis(); }
  };
