...
bool printJobOngoing() {
  return /* print_job_timer.isRunning() || */ IS_SD_PRINTING() || IS_SD_PAUSED();
}
...
#ifdef USE_FREERTOS
extern void Marlin_Kill_FromISR(PGM_P const lcd_error, PGM_P const lcd_component, const bool steppers_off);
extern uint8_t marlin_nest_ISR;
  if(marlin_nest_ISR){
	  Marlin_Kill_FromISR(lcd_error,lcd_component,steppers_off);
	  return;
  }
#endif
...
#ifdef USE_FREERTOS
	//void MarlinUI_Process();
	extern void Marlin_Thread_Suspend();
#endif
...
#if HAS_BUZZER
   SETUP_RUN(buzzer.setup());  // setup the Beeper
#endif

#if ENABLED(SDSUPPORT) && PIN_EXISTS(SD_DETECT)
   SETUP_RUN(setup_sddetectpin());
#endif
