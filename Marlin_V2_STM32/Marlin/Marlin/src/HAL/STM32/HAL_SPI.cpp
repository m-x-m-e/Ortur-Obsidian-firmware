...
#if ENABLED(CUSTOM_SPI)

#include "../../libs/softspi.h"
#include "hwspi.h"
...
//SoftSPI<MISO_PIN,MOSI_PIN,SCK_PIN> SDSPI;
HWSPI<MISO_PIN,MOSI_PIN,SCK_PIN> SDSPI;
...
#ifdef USE_SPI_DMA
	__attribute__ ((aligned(4))) static uint8_t revdata[512]={0};
	   SDSPI.dmaTransfer(&token,(uint8_t*)revdata,1);
	   SDSPI.dmaTransfer((uint8_t*)buf,revdata,512);
#else
	SDSPI.send(token);
	  for (int i = 0; i < 512; i++) {
		  SDSPI.send(buf[i]);
