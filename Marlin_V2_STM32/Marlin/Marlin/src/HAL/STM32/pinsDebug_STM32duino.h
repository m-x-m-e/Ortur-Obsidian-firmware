...
uint8_t get_pin_mode(const pin_t Ard_num) {
  ...
  uint64_t mode_all = 0;
  uint8_t crhl_bits = 0;
  uint8_t confy_bits = 0;
  uint8_t modely_bits = 0;
  uint8_t model_pin = 0;

  switch (PORT_ALPHA(dp)) {
      case 'A' : mode_all = *((uint64_t*)(&GPIOA->CRL)); break;
      case 'B' : mode_all = *((uint64_t*)(&GPIOB->CRL)); break;
      case 'C' : mode_all = *((uint64_t*)(&GPIOC->CRL)); break;
      case 'D' : mode_all = *((uint64_t*)(&GPIOD->CRL)); break;
      #ifdef PE_0
        case 'E' : mode_all = *((uint64_t*)(&GPIOE->CRL)); break;
      #elif defined(PF_0)
        case 'F' : mode_all = *((uint64_t*)(&GPIOF->CRL)); break;
      #elif defined(PG_0)
        case 'G' : mode_all = *((uint64_t*)(&GPIOG->CRL)); break;
      #elif defined(PH_0)
        case 'H' : mode_all = *((uint64_t*)(&GPIOH->CRL)); break;
      #elif defined(PI_0)
        case 'I' : mode_all = *((uint64_t*)(&GPIOI->CRL)); break;
      #elif defined(PJ_0)
        case 'J' : mode_all = *((uint64_t*)(&GPIOJ->CRL)); break;
      #elif defined(PK_0)
        case 'K' : mode_all = *((uint64_t*)(&GPIOK->CRL)); break;
      #elif defined(PL_0)
        case 'L' : mode_all = *((uint64_t*)(&GPIOL->CRL)); break;
      #endif
    }

  crhl_bits =  (mode_all >> (4 * uint8_t(PIN_NUM(dp)))) & 0x0F;
  confy_bits  = (crhl_bits >> 2) & 0x3;
  modely_bits = crhl_bits & 0x3;
  model_pin= 0;

  if(modely_bits) {
	  //output
	  if(confy_bits >= 2)
		  model_pin = MODE_PIN_ALT;
	  else
		  model_pin = MODE_PIN_OUTPUT;
  }
  else {
	  //input
	  if(confy_bits)
	  	  model_pin = MODE_PIN_ANALOG;
	  else
		  model_pin = MODE_PIN_INPUT;
  }
    return model_pin;
#endif
#ifdef STM32F4
...
#ifdef STM32F1
	//NOTE: STM32F1 can not confirm Alt Function
	SERIAL_ECHOPGM("Alt Function: - can not confirm");
#endif
#ifdef STM32F4
    uint32_t alt_all = 0;
