...
#if LV_COLOR_16_SWAP
            uint16_t __green = (uint16_t) ((uint16_t) (canvas_buf_color[j].green_h<<3 + canvas_buf_color[j].green_l<<3) * ((copy_buf_color[j].green_h<<3) + copy_buf_color[j].green_l)) >> 8;
            canvas_buf_color[j].green_h = (__green >> 3) & 0b111;
            canvas_buf_color[j].green_l = __green & 0b111;
#else
