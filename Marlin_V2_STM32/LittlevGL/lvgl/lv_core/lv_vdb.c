...
#ifndef LV_VDB_ATTRIBUTE
#define LV_VDB_ATTRIBUTE
#endif
...
static uint8_t LV_VDB_ATTRIBUTE vdb_buf[LV_VDB_SIZE_IN_BYTES];
...
#  if LV_VDB_ADR == 0
/*If the buffer address is not specified  simply allocate it*/
static uint8_t LV_VDB_ATTRIBUTE vdb_buf1[LV_VDB_SIZE_IN_BYTES];
static uint8_t LV_VDB_ATTRIBUTE vdb_buf2[LV_VDB_SIZE_IN_BYTES];
...
//NOTE: 双缓冲机制时,将是否进行下一次的刷新的判断放在刷新函数中
